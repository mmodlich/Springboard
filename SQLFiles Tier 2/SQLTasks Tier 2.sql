...
SELECT COUNT(*) AS count_fee_facilities
FROM Facilities
WHERE membercost = 0;
...
ELSE "cheap" END AS cost_analysis
FROM Facilities;
...
SELECT DISTINCT m.memid, f.facid, f.name, 
...
SELECT CONCAT_WS(' ', m1.surname, m1.firstname) AS member, 
    CONCAT_WS(' ', m2.surname, m2.firstname) AS recommendedby
FROM Members AS m1
